...
class ReferenceVectors:
    ...
    def adapt_moead_pre(self, mapping_point, roi_size):
        new_rvs = np.zeros((self.number_of_vectors, self.number_of_objectives))
        new_rvs[0] = mapping_point
        if (self.number_of_objectives < 3):
            step_len = roi_size/(self.number_of_vectors - 1)
            for i in range(0, self.number_of_vectors):
                new_rvs[i][0] = mapping_point[0] - roi_size/ 2 + step_len * i
                new_rvs[i][1] = 1.0 - new_rvs[i][0]
        else:
            boundary_vectors = self.get_boundary_points_moead_pre(mapping_point, roi_size)
            for i in range(0, self.number_of_objectives):
                new_rvs[i+1][:] = boundary_vectors[i][:]
            num_vectors = self.number_of_objectives + 1
            #init_rvs_moead_pre()
            k = 0
            for i in range (0, num_vectors):
                for j in range (i + 1, num_vectors):
                    if(k + end < limit_size):
                        new_rvs[k + end] = get_lamb_moead_pre(new_rvs[i], new_rvs[j], nobj)
                        k = k+1
                    else:
                        break
            num_vectors = num_vectors + k
            while(new_rvs < self.number_of_vectors):
                print()


            
    def get_lamb_moead_pre(self, a, b):
        lamb = np.zeros(self.number_of_objectives)
        for i in range(0, self.number_of_objectives):
            lamb[i] = (a[i] + b[i])/2.0
        return lamb

    #def init_rvs_moead_pre(self, old, end, limit_size):
    #    k = 0
    #    for i in range (0, end):
    #        for j in range (i + 1, end):
    #            if(k + end < limit_size):
    #                old[k + end] = get_lamb_moead_pre(old[i], old[j], nobj)
    #                k = k+1
    #            else:
    #                break

    def get_boundary_points_moead_pre(self, mapping_point, roi_size):
        bj1 = np.zeros(self.number_of_objectives)
        bj2 = np.zeros((self.number_of_objectives, self.number_of_objectives))
        for i in range(0, self.number_of_objectives):
            for j in range(0, self.number_of_objectives):
                if(j == i):
                    bj1[i] = bj1[i] + (1.0 - mapping_point[j])**2
                else:
                    bj1[i] = bj1[i] + (mapping_point[j] ** 2)
            bj1[i] = np.sqrt(bj1[i])

        for i in range(0, self.number_of_objectives):
            for j in range(0, self.number_of_objectives):
                if(j == i):
                    bj2[i][j] = roi_size * 1.0 / bj1[i] + (1.0 - roi_size / bj1[i]) * mapping_point[j]
                else:
                    bj2[i][j] = (1.0 - roi_size / bj1[i]) * mapping_point[j]

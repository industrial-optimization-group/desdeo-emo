...
class BaseDecompositionEA(BaseEA):
    ...
    def request_plot(self) -> SimplePlotRequest:
        ...
        dimensions_data.loc["ideal"] = self.population.ideal_fitness_val
        dimensions_data.loc["nadir"] = self.population.ideal_fitness_val
        data = pd.DataFrame(
...
class BaseAprioriMOEA(BaseDecompositionEA):
    def __init__(
        self,
        problem: MOProblem,

...
class MOEA_D_NUMS(BaseDecompositionEA):
    ...
    def __init__(  # parameters of the class
        ...
        #self._global_ideal_point = global_ideal_point
    ...
    def manage_preferences(self, preference=None):
        if isinstance(preference, ReferencePointPreference):
            self.reference_vectors.adapt_nums(np.array(preference.response.values[0]), self.flag, self.roi_size, self._ideal_point)
    ...
    def iterate(self, preference=None) -> Tuple:
        ...
        self.population = Population(self.problem, self.population_size, None, self.use_surrogates)
        self._function_evaluation_count += self.population_size
        #self.population.update_ideal()
        self._ideal_point = np.copy(self.population.ideal_fitness_val)
